...
<style>
  ...
  h1{color:#1565c0;margin-bottom:6px}
  .counter{color:#1565c0;font-weight:bold;margin-bottom:10px}
  button{
    background:#1976d2;
    color:#fff;
    border:none;
  ...
  .info{font-size:.9em;color:#555;margin-top:10px}
</style>
</head>
...
const MAX_DAY = 30;
const STORAGE_KEY = "gym_dada_progress_v2";
...
function render(){
  ...
  document.getElementById("progress").innerText =
    `Hari ${data.day} • Streak ${data.streak}🔥`;

  if(now < data.lockedUntil){
    btn.style.display="none";
    ex.innerHTML = `<b>Tunggu ${format(data.lockedUntil-now)}</b>`;
  ...
  if(data.day > MAX_DAY){
    btn.style.display="none";
    ex.innerHTML="🎉 Program dada selesai!";
